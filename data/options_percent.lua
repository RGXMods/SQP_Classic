...
function SQP:CreatePercentOptions(content)
    ...
    local percentAnimIntensityReset = self:CreateInlineResetButton(leftColumn, function()
        ...
        percentAnimIntensityLabel:SetText("Intensity: 100%")
        ActivatePercent()
        SQP:RefreshAllNameplates()
    end)
    ...
    percentAnimIntensitySlider:SetScript("OnValueChanged", function(self, newVal)
        ...
        percentAnimIntensityLabel:SetText(string.format("Intensity: %d%%", newVal))
        ActivatePercent()
        SQP:RefreshAllNameplates()
    end)
    yOffset = yOffset - 44
    ...
    resetBtn:SetScript("OnClick", function()
        ...
        if oc.percentAnimationIntensityLabel then
            oc.percentAnimationIntensityLabel:SetText(string.format("Intensity: %d%%", D.percentAnimationIntensity))
        end
